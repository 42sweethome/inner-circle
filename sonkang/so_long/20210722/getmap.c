...
int		draw(t_info *info, int y, int x, t_data tex)
{
	...
	while (++height < tex.img_height)
	{
		...
		{
			info->fimg.addr[tex.img_width * tex.img_height * (x + info->map.row * y) + height * tex.img_width + width] = tex.addr[height * tex.img_width + width];
		}
		width = -1;
	}
	return (0);
...
void	get_texture(t_info *info)
{
	...
	info->tex[4].img = mlx_xpm_file_to_image(info->win.mlx, "./texture/grass.xpm", &info->tex[4].img_width, &info->tex[4].img_height);
	info->tex[0].addr = mlx_get_data_addr(info->tex[0].img, &info->tex[0].bits_per_pixel, &info->tex[0].line_length, &info->tex[0].endian);
	info->tex[1].addr = mlx_get_data_addr(info->tex[1].img, &info->tex[1].bits_per_pixel, &info->tex[1].line_length, &info->tex[1].endian);
	info->tex[2].addr = mlx_get_data_addr(info->tex[2].img, &info->tex[2].bits_per_pixel, &info->tex[2].line_length, &info->tex[2].endian);
	info->tex[3].addr = mlx_get_data_addr(info->tex[3].img, &info->tex[3].bits_per_pixel, &info->tex[3].line_length, &info->tex[3].endian);
	info->tex[4].addr = mlx_get_data_addr(info->tex[4].img, &info->tex[4].bits_per_pixel, &info->tex[4].line_length, &info->tex[4].endian);
}
...
void	show_win(t_info *info)
{
	...
	info->fimg.addr = mlx_get_data_addr(info->fimg.img, &info->fimg.bits_per_pixel, &info->fimg.line_length, &info->fimg.endian);
	get_texture(info);
