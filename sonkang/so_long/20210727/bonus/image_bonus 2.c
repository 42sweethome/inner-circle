...
void	get_texture(t_info *info)
{
	...
	"./texture/right1.xpm", &info->tex[4].img_width, &info->tex[4].img_height);
	info->tex[5].img = mlx_xpm_file_to_image(info->win.mlx, \
	"./texture/right2.xpm", &info->tex[5].img_width, &info->tex[5].img_height);
	info->tex[6].img = mlx_xpm_file_to_image(info->win.mlx, \
	"./texture/left1.xpm", &info->tex[6].img_width, &info->tex[6].img_height);
	info->tex[7].img = mlx_xpm_file_to_image(info->win.mlx, \
	"./texture/left2.xpm", &info->tex[7].img_width, &info->tex[7].img_height);
	...
	"./texture/ghost_right.xpm", &info->tex[12].img_width, &info->tex[12].img_height);
	info->tex[13].img = mlx_xpm_file_to_image(info->win.mlx, \
	"./texture/ghost_left.xpm", &info->tex[13].img_width, &info->tex[13].img_height);
	get_texture2(info);
}
...
int	img_conv(t_info *info)
{
	...
	while (++y < info->map.row)
	{
		...
		while (++x < info->map.col)
		{
			...
			{
				draw(info, y, x, info->tex[12 + info->map.v_d]);
			}
		}
	}
