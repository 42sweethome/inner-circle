...
/*   Updated: 2021/08/31 23:49:26 by sonkang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	space_split(char *s, char space, t_mini *mini)
{
	...
	//if (s == 0)
	//	return (mini->err.malloc); parsing에서 only_space로 처리해줌
	...
	if (!mini->buf || num == (size_t)mini->err.malloc) //num -1일때 여기서 에러처리하는게 맞는지
		return (mini->err.malloc);
