...
/*   Updated: 2021/05/14 16:52:03 by junghan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		rerot_ab(t_link *st_a, t_link *st_b, t_info info)//
{
	int		i;
...
int			sort_a(t_link *st_a, t_link *st_b, int range, t_info *info)
{
	int		*num;
	int		ret;
	int		pivot[3];
	...
	rerot_ab(st_a, st_b, reinfo);
	sort_a(st_a, st_b, info.ra, info);
	//rerot_b(st_b, info);
	sort_b(st_a, st_b, info.rb, info);
	sort_b(st_a, st_b, info.pb, info);
	return (1);
}
