...
/*   Updated: 2021/05/13 17:57:01 by junghan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
static void		under_4(t_link *st_a, t_info *info, int *ret)
{
	int		a;
	int		b;
	int		c;
...
static void		under_5(t_link *st_a, t_link *st_b, t_info *info, int *ret)
{
	if (st_a->head->next->next->next->value < st_a->head->next->value
			&& st_a->head->next->value < st_a->head->next->next->next->next->value
			&& st_a->head->next->next->next->next->value < st_a->head->next->next->value
	...
	if (st_a->head->next->next->value <  st_a->head->next->next->next->next->value
			&&  st_a->head->next->next->next->next->value < st_a->head->next->next->next->value
