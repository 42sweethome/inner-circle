...
/*   Updated: 2021/07/23 11:21:42 by junghan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_texture(t_info *info)
{
	...
	info->tex[2].img = mlx_xpm_file_to_image(info->win.mlx, "./texture/ghost.xpm", &info->tex[2].img_width, &info->tex[2].img_height);
	info->tex[3].img = mlx_xpm_file_to_image(info->win.mlx, "./texture/grass.xpm", &info->tex[3].img_width, &info->tex[3].img_height);
	info->tex[4].img = mlx_xpm_file_to_image(info->win.mlx, "./texture/ghost.xpm", &info->tex[4].img_width, &info->tex[4].img_height);
	...
	printf("bpp = %d\n", info->tex[0].bits_per_pixel);
	printf("ll = %d\n", info->tex[0].line_length);
	printf("end = %d\n", info->tex[0].endian);
	...
	{
		j = 0;
		while (j < info->tex[k].img_height)
		{
			i = 0;
			while (i < info->tex[k].img_width)
			{
				info->tex[k].addr[j * (info->tex[k].img_width / 4) + i] = \
					info->tex[k].addr[j * (info->tex[k].img_width) + i]; //+ \
					//(j * info->tex[k].line_length + i * (info->tex[k].bits_per_pixel / 8));
				i++;
			}
			j++;
		}
